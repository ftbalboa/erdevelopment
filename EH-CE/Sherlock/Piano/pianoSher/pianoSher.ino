/* PIANO_06
 * 
 * MEGA, SENSORES DE RANURA Y RELE
 * 
 */
 
//EDITABLE
#define SPRINT true
#define NEED_REPOSO false
#define RELE  2
#define A     3
#define B     4
#define C     5
#define D     6
#define E     7


#define DS    10
const int N_INPUTS = 5;
const int INPUTS[N_INPUTS] = {A,B,C,D,E};
const int INPUTS_ISTATE[N_INPUTS] = {1,1,1,1,1};
const int N_OUTPUTS = 1;
const int OUTPUTS[N_OUTPUTS] = {RELE};
const int OUTPUTS_ISTATE[N_OUTPUTS] = {1};

const int LARGO_SECUENCIA = /*16*/8;
const int SECUENCIA[LARGO_SECUENCIA]={D,B,B,A,B,C,B,E};

int lugar = 0;

boolean ret_C1 = false;
boolean ret_CS = false;
boolean ret_D1 = false;
boolean ret_E1 = false;
boolean ret_F1 = false;
boolean ret_G1 = false;
boolean ret_A1 = false;
boolean ret_B1 = false;
boolean ret_C2 = false;

//NO EDITABLE
boolean reposo = false;
boolean activo = false;
boolean resuelto = false;
unsigned long timer_print = 0;
////////////////////////////////////SETUP///////////////////////////////
//NO EDITABLE
void setup() {
  if(SPRINT){Serial.begin(9600);} 
//declara entradas
  for(int a = 0;a < N_INPUTS;a++){ 
    pinMode(INPUTS[a] , INPUT);
    digitalWrite(INPUTS[a] , INPUTS_ISTATE[a]);
  }
//declara salidas
  for(int a = 0;a < N_OUTPUTS;a++){ 
    pinMode(OUTPUTS[a] , OUTPUT);
    digitalWrite(OUTPUTS[a] , OUTPUTS_ISTATE[a]);
  }
//si necesita reposo
  if(NEED_REPOSO){reposo=true;activo=false;resuelto=false;}else{reposo=false;activo=true;resuelto=false;}
//EDITABLE
}

///////////////////////////////////////LOOP//////////////////////////////////////////
void loop() {
  if_reposo();
  if_activo();
  if_resuelto();
  showme();
}

////////////////////////////////////////FUNCIONES///////////////////////////////////
//EDITABLE
void if_reposo()
{
  if(reposo){
  }
}

void if_activo()
{
  if(activo){

    if(check(0, A ,DS)){
      if(ret_C1){}else{ret_C1=true; if(A == SECUENCIA[lugar]){lugar++;}else{lugar = 0;}}
    }else{
      ret_C1=false;}
    if(lugar == LARGO_SECUENCIA){lugar = 0; activo = false; resuelto = true;}
    if(check(0, B ,DS)){
      if(ret_CS){}else{ret_CS=true; if(B == SECUENCIA[lugar]){lugar++;}else{lugar = 0;}}
    }else{
      ret_CS=false;}
    if(lugar == LARGO_SECUENCIA){lugar = 0; activo = false; resuelto = true;}
    if(check(0, C ,DS)){
      if(ret_D1){}else{ret_D1=true; if(C == SECUENCIA[lugar]){lugar++;}else{lugar = 0;}}
    }else{
      ret_D1=false;}
    if(lugar == LARGO_SECUENCIA){lugar = 0; activo = false; resuelto = true;}
    if(check(0, D ,DS)){
      if(ret_E1){}else{ret_E1=true; if(D == SECUENCIA[lugar]){lugar++;}else{lugar = 0;}}
    }else{
      ret_E1=false;}
    if(lugar == LARGO_SECUENCIA){lugar = 0; activo = false; resuelto = true;}
    if(check(0, E ,DS)){
      if(ret_F1){}else{ret_F1=true; if(E == SECUENCIA[lugar]){lugar++;}else{lugar = 0;}}
    }else{
      ret_F1=false;}
    if(lugar == LARGO_SECUENCIA){lugar = 0; activo = false; resuelto = true;}

  }
}

void if_resuelto()
{
  if(resuelto){
    digitalWrite(RELE, LOW);
    delay(5000);
    digitalWrite(RELE, HIGH);
    resuelto = false;
    activo = true;
  }
}

void showme ()
{
  if(SPRINT && millis() - timer_print > 1000)
  {
    Serial.println("------NUEVA MEDICION------");timer_print = millis();
    Serial.print("A: ");
    Serial.println(ret_C1);
    Serial.print("B: ");
    Serial.println(ret_CS);
    Serial.print("C1: ");
    Serial.println(ret_D1);
    Serial.print("D1: ");
    Serial.println(ret_E1);
    Serial.print("E1: ");
    Serial.println(ret_F1);

    Serial.print("Lugar: ");
    Serial.println(lugar);
    }
  }

//NO EDITABLE
//comprueba el estado de un sensor
boolean check (boolean cual, int entrada, int ds)
{
  boolean resu = false;
  int contador = 0;  
  if(cual){ while(digitalRead(entrada) == 1 && resu == false){delay(ds);contador++;if(contador == 5){resu = true;}}
  }else{    while(digitalRead(entrada) == 0 && resu == false){delay(ds);contador++;if(contador == 5){resu = true;}}}  
  return resu; 
}
