...
void eth_update()
{
  if ((millis() - contador) >= 500) { // Esto lo puedes dejar como estaba, pero creo que es más ilustrativo así
    contador = millis();
    if(eth_on == false){
      if (client.connect("192.168.0.110", 23)){eth_on = true;}else{}
      }else{
        char c = client.read();
        while(c != -1){
...
void if_activo()
{
  ...
  ////////EDITAR A COSAS A HACER CUANDO SE ACTIVA/////
    ...
    boolean pote3 = false;
    boolean linea = false;
    if(analog0()>LIM0-CHANGUI && analog0()<LIM0+CHANGUI){pote0=true;} 
    if(analog1()>LIM1-CHANGUI && analog1()<LIM1+CHANGUI){pote1=true;}
    if(analog2()>LIM2-CHANGUI && analog2()<LIM2+CHANGUI){pote2=true;} 
    if(analog3()>LIM3-CHANGUI && analog3()<LIM3+CHANGUI){pote3=true;}
    if(digitalRead(LLAVE) == 0){delay(20); if(digitalRead(LLAVE) == 0){linea = true;}}
