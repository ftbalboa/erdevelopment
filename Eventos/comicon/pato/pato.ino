...
const int INPUTS[N_INPUTS] = {CB,BLLAVE,RSPATO,BFINAL};
const int INPUTS_ISTATE[N_INPUTS] = {1,1,1,1};
const int N_OUTPUTS = 5;
const int OUTPUTS[N_OUTPUTS] = {CL,RLUZ,RCM1,RCMFIN,RSELUZ};
...
int game(int paso_actual, boolean change)
{
  switch(paso_actual){
    ...
    case 0:
      if(change){while(check(0,BLLAVE,DS)){};reset_game();}
      if(check(0,BLLAVE,DS)){paso_actual++;}
      break;

    case 1:{
      if(change){digitalWrite(RLUZ,LOW);}
      int hey = 0;
      while(check(0,RSPATO,DS) && hey < 30){hey++;}if(hey > 29){paso_actual++;}
      break;}

     case 2:
      if(change){digitalWrite(RCM1,LOW);digitalWrite(RSELUZ,LOW);}
      if(check(0,BFINAL,DS)){paso_actual++;}
      break;
